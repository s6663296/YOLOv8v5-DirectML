...
class _Config:
    def __init__(self):
        self.default = {
            ...
            "terminal_compact_logs": True,
            "terminal_dedupe_seconds": 3.0,
            "aim_range": 150,
            "aim_width": 300,
            "aim_height": 300,
            "aimBot": True,
            "aimbot_hotkey_enabled": False,
            "aimbot_toggle_hotkey": "VK_F8",
            "confidence": 0.3,
            "iou_threshold": 0.45,
            "aim_speed_x": 6.7,
            "aim_speed_y": 8.3,
            ...
            "capture_fps": 240,
            "capture_source": "mss",  # 截圖來源: "mss" 螢幕截圖 或 "dxgi" DXGI高效截圖 或 "obs" OBS UDP 接收
            "capture_monitor_index": 1,  # 指定擷取顯示器（1=第一個顯示器）
            "obs_ip": "",  # UDP 監聽 IP（空字串表示監聽所有介面）
            "obs_port": 1234,  # UDP 監聽端口

            "inference_backend": "dml",  # 推理後端: "dml" DirectML, "cuda" CUDA, "cpu" CPU, "trt" TensorRT
            "yolo_version": "auto",
            
            # --- 影像前處理設定 ---
            "preprocess_bgr_to_rgb": True,
            "preprocess_enable_clahe": True,
            "preprocess_clahe_clip_limit": 2.0,
            "preprocess_clahe_tile_size": 8,
            "preprocess_enable_gamma": True,
            "preprocess_gamma_value": 1.0,
            "preprocess_enable_white_balance": True,
            "preprocess_wb_clip_factor": 0.5,
        }
