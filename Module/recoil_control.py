...
class RecoilControl:
    ...
    def set_config(self, enabled, x_strength, y_strength, delay, mouse_move_mode, trigger_keys_str):
        is_state_change = self.enabled != enabled
    ...
    def _parse_trigger_keys(self, keys_str):
        ...
        for key_name in key_names:
            if key_name:
                vk_code = get_key_code_vk(key_name)
                if vk_code != "UNKNOWN":
        ...
        while not self.exit_event.is_set():
            try:
                if self.trigger_keys:
                    all_keys_pressed = all(keyboard_listener.is_pressed(key) for key in self.trigger_keys)
                    if all_keys_pressed:
